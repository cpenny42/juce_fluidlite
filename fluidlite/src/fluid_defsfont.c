...
{
    preset_callback = (void (*) (unsigned int, unsigned int, char*)) callback;
}
...
int fluid_defsfont_load(fluid_defsfont_t* sfont, const char* file)
{
  ...
  sfont->filename = (char*) FLUID_MALLOC(1 + FLUID_STRLEN(file));
  if (sfont->filename == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Out of memory");
    return FLUID_FAILED;
  }
  ...
  if (sfdata == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Couldn't load soundfont file");
    return FLUID_FAILED;
  }
...
fluid_defsfont_load_sampledata(fluid_defsfont_t* sfont)
{
  ...
  if (fd == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Can't open soundfont file");
    return FLUID_FAILED;
  }
  if (FLUID_FSEEK(fd, sfont->samplepos, SEEK_SET) == -1) {
    perror("error");
    FLUID_LOG(FLUID_ERR, (char*) "Failed to seek position in data file");
    return FLUID_FAILED;
  }
  ...
  if (FLUID_FREAD(sfont->sampledata, 1, sfont->samplesize, fd) < sfont->samplesize) {
    FLUID_LOG(FLUID_ERR, (char*) "Failed to read sample data");
    return FLUID_FAILED;
  }
  FLUID_FCLOSE(fd);
  ...
  for (list = sfont->sample; list; list = fluid_list_next(list)) {
    ...
    if (FLUID_STRCMP(sample->name, s) == 0) {
        ...
        if (sample->sampletype & FLUID_SAMPLETYPE_OGG_VORBIS)
        {
            ...
            vorbisData.datasize = (int) (sample->end + 1 - sample->start);
            if (ov_open_callbacks(&vorbisData, &vf, 0, 0, ovCallbacks) == 0)
            {
                ...
                do {
                    ...
                    sampledata_ogg = (short*) realloc(sampledata_ogg, (size_t) (sampledata_size + numberRead));
                    if(numberRead>0)
                    {
                        memcpy((char*)(sampledata_ogg)+sampledata_size,buffer, (size_t) numberRead);
          ...
          sample->start = 0;
          sample->end = (unsigned int) (sampledata_size - 1);
...
new_fluid_preset_zone(char *name)
{
  ...
  zone->name = (char*) FLUID_MALLOC((size_t)size);
  if (zone->name == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Out of memory");
    FLUID_FREE(zone);
    return NULL;
...
/*    } */
  ...
  if ((sfzone->instsamp != NULL) && (sfzone->instsamp->data != NULL)) {
    ...
    if (zone->sample == NULL) {
      FLUID_LOG(FLUID_ERR, (char*) "Couldn't find sample name");
      return FLUID_FAILED;
    }
  }
...
fluid_sample_import_sfont(fluid_sample_t* sample, SFSample* sfsample, fluid_defsfont_t* sfont)
{
  ...
  if (sample->sampletype & FLUID_SAMPLETYPE_ROM) {
    sample->valid = 0;
    FLUID_LOG(FLUID_WARN, (char*) "Ignoring sample %s: can't use ROM samples", sample->name);
  }
  if (sample->end - sample->start < 8) {
    sample->valid = 0;
    FLUID_LOG(FLUID_WARN, (char*) "Ignoring sample %s: too few sample data points", sample->name);
  } else {
...
#define READD(var,fd)		G_STMT_START {		\
...
#endif
#ifdef WORDS_BIGENDIAN
    #define READDU(var,fd)        G_STMT_START {        \
    unsigned int _temp;                    \
    if (!safe_fread(&_temp, 4, fd))            \
    return(FAIL);                    \
    var = (unsigned int) GINT32_FROM_BE(_temp);            \
    } G_STMT_END
#else
    #define READDU(var,fd)        G_STMT_START {        \
    unsigned int _temp;                    \
    if (!safe_fread(&_temp, 4, fd))            \
    return(FAIL);                    \
    var = (unsigned int) GINT32_FROM_LE(_temp);            \
    } G_STMT_END
#endif
#ifdef WORDS_BIGENDIAN
...
#endif
#ifdef WORDS_BIGENDIAN
#define READWU(var,fd)        G_STMT_START {        \
    unsigned short _temp;                    \
    if (!safe_fread(&_temp, 2, fd))            \
    return(FAIL);                    \
    var = (unsigned short) GINT16_FROM_BE(_temp);            \
    } G_STMT_END
#else
#define READWU(var,fd)        G_STMT_START {        \
    unsigned short _temp;                    \
    if (!safe_fread(&_temp, 2, fd))            \
    return(FAIL);                    \
    var = (unsigned short) GINT16_FROM_LE(_temp);            \
    } G_STMT_END
#endif
...
sfload_file (const char * fname)
{
  ...
  if (!(fd = fopen (fname, "rb")))
    {
      FLUID_LOG (FLUID_ERR, (char*)  _("Unable to open file \"%s\""), fname);
      return (NULL);
    }
  ...
  if (!err && fseek (fd, 0L, SEEK_END) == -1)
    {				/* seek to end of file */
      err = TRUE;
      FLUID_LOG (FLUID_ERR, (char*) _("Seek to end of file failed"));
    }
  if (!err && (fsize = (int) ftell (fd)) == -1)
    {				/* position = size */
      err = TRUE;
      FLUID_LOG (FLUID_ERR, (char*) _("Get end of file position failed"));
    }
  if (!err)
    rewind (fd);
...
load_body (unsigned int size, SFData * sf, FILE * fd)
{
  ...
  if (chunkid (chunk.id) != RIFF_ID) {	/* error if not RIFF */
    FLUID_LOG (FLUID_ERR, (char*) _("Not a RIFF file"));
    return (FAIL);
  }

  READID (&chunk.id, fd);	/* load file ID */
  if (chunkid (chunk.id) != SFBK_ID) {	/* error if not SFBK_ID */
    FLUID_LOG (FLUID_ERR, (char*) _("Not a sound font file"));
    return (FAIL);
  }

  if (chunk.size != size - 8) {
    gerr (ErrCorr, (char*) _("Sound font file size mismatch"));
    return (FAIL);
  }
  ...
  if (chunkid (chunk.id) != INFO_ID)
    return (gerr (ErrCorr, (char*) _("Invalid ID found when expecting INFO chunk")));
  if (!process_info ((int) chunk.size, sf, fd))
    return (FAIL);
  ...
  if (chunkid (chunk.id) != SDTA_ID)
    return (gerr (ErrCorr,
	(char*) _("Invalid ID found when expecting SAMPLE chunk")));
  if (!process_sdta ((int) chunk.size, sf, fd))
    return (FAIL);
  ...
  if (chunkid (chunk.id) != PDTA_ID)
    return (gerr (ErrCorr, (char*) _("Invalid ID found when expecting HYDRA chunk")));
  if (!process_pdta ((int) chunk.size, sf, fd))
    return (FAIL);
...
read_listchunk (SFChunk * chunk, FILE * fd)
{
  ...
  if (chunkid (chunk->id) != LIST_ID)	/* error if ! list chunk */
    return (gerr (ErrCorr, (char*) _("Invalid chunk id in level 0 parse")));
  ...
  while (size > 0)
    {
      ...
      id = (unsigned char) chunkid (chunk.id);

      if (id == IFIL_ID)
	{			/* sound font version chunk? */
	  if (chunk.size != 4)
	    return (gerr (ErrCorr,
		(char*) _("Sound font version info chunk has invalid size")));
	  ...
	  if (sf->version.major < 2) {
	    FLUID_LOG (FLUID_ERR,
		      (char*) _("Sound font version is %d.%d which is not"
	  ...
	  if (sf->version.major > 2) {
	    FLUID_LOG (FLUID_WARN,
		      (char*) _("Sound font version is %d.%d which is newer than"
	...
	{			/* ROM version chunk? */
	  if (chunk.size != 4)
	    return (gerr (ErrCorr,
		(char*) _("ROM version info chunk has invalid size")));
      ...
      else if (id != UNKN_ID)
	{
	  if ((id != ICMT_ID && chunk.size > 256) || (chunk.size > 65536)
	    || (chunk.size % 2))
	    return (gerr (ErrCorr,
		(char*) _("INFO sub chunk %.4s has invalid chunk size"
	  ...
	  *(unsigned char *) item = id;
	  if (!safe_fread (&item[1], (int) chunk.size, fd))
	    return (FAIL);
	...
	return (gerr (ErrCorr, (char*) _("Invalid chunk id in INFO chunk")));
      size -= chunk.size;
    }

  if (size < 0)
    return (gerr (ErrCorr, (char*) _("INFO chunk size mismatch")));
...
process_sdta (int size, SFData * sf, FILE * fd)
{
  ...
  if (chunkid (chunk.id) != SMPL_ID)
    return (gerr (ErrCorr,
	(char*) _("Expected SMPL chunk found invalid id instead")));

  if ((size - (int) chunk.size) != 0)
    return (gerr (ErrCorr, (char*) _("SDTA chunk size mismatch")));

  /* sample data follows */
  sf->samplepos = (unsigned int) ftell (fd);
  ...
  int * size, FILE * fd)
{
  ...
  *size -= 8;
  ...
  if ((id = (unsigned int) chunkid (chunk->id)) != expid)
    return (gerr (ErrCorr, (char*) _("Expected"
	  " PDTA sub-chunk \"%.4s\" found invalid id instead"), expstr));

  if (chunk->size % reclen)	/* valid chunk size? */
    return (gerr (ErrCorr,
	(char*) _("\"%.4s\" chunk size is not a multiple of %d bytes"), expstr,
	reclen));
  if ((*size -= chunk->size) < 0)
    return (gerr (ErrCorr,
	(char*) _("\"%.4s\" chunk size exceeds remaining PDTA chunk size"), expstr));
  return (OK);
}
...
load_phdr (int size, SFData * sf, FILE * fd)
{
  ...
  if (size % SFPHDRSIZE || size == 0)
    return (gerr (ErrCorr, (char*) _("Preset header chunk size is invalid")));

  i = size / SFPHDRSIZE - 1;
  if (i == 0)
    {				/* at least one preset + term record */
      FLUID_LOG (FLUID_WARN, (char*) _("File contains no presets"));
      FSKIP (SFPHDRSIZE, fd);
      return (OK);
    }
  ...
  for (; i > 0; i--)
	...
	{			/* not first preset? */
	  if (zndx < pzndx)
	    return (gerr (ErrCorr, (char*) _("Preset header indices not monotonic")));
	  i2 = zndx - pzndx;
	  while (i2--)
	    {
	...
	FLUID_LOG (FLUID_WARN, (char*) _("%d preset zones not referenced, discarding"), zndx);
      pr = p;			/* update preset ptr */
      pzndx = zndx;
    }

  FSKIP (24, fd);
  READWU (zndx, fd);		/* Read terminal generator index */
  FSKIP (12, fd);
...
load_pbag (int size, SFData * sf, FILE * fd)
{
  ...
  if (size % SFBAGSIZE || size == 0)	/* size is multiple of SFBAGSIZE? */
    return (gerr (ErrCorr, (char*) _("Preset bag chunk size is invalid")));
	...
	{			/* traverse preset's zones */
	  ...
	  if (pz)
	    {			/* if not first zone */
	      if (genndx < pgenndx)
		return (gerr (ErrCorr,
		    (char*) _("Preset bag generator indices not monotonic")));
	      if (modndx < pmodndx)
		return (gerr (ErrCorr,
		    (char*) _("Preset bag modulator indices not monotonic")));
	      i = genndx - pgenndx;
	      while (i--)
  ...
  if (!pz)
    {
      if (genndx > 0)
	FLUID_LOG (FLUID_WARN, (char*) _("No preset generators and terminal index not 0"));
      if (modndx > 0)
	FLUID_LOG (FLUID_WARN, (char*) _("No preset modulators and terminal index not 0"));
      return (OK);
    }

  if (genndx < pgenndx)
    return (gerr (ErrCorr, (char*) _("Preset bag generator indices not monotonic")));
  if (modndx < pmodndx)
    return (gerr (ErrCorr, (char*) _("Preset bag modulator indices not monotonic")));
  i = genndx - pgenndx;
  while (i--)
    pz->gen = fluid_list_prepend (pz->gen, NULL);
  i = modndx - pmodndx;
...
load_pmod (int size, SFData * sf, FILE * fd)
{
	...
	{			/* traverse this preset's zones */
	  ...
	  while (p3)
	    {			/* load zone's modulators */
	      if ((size -= SFMODSIZE) < 0)
		return (gerr (ErrCorr,
		    (char*) _("Preset modulator chunk size mismatch")));
	      m = FLUID_NEW (SFMod);
	      p3->data = m;
	      READWU (m->src, fd);
	      READWU (m->dest, fd);
  ...
  size -= SFMODSIZE;
  if (size != 0)
    return (gerr (ErrCorr, (char*) _("Preset modulator chunk size mismatch")));
...
load_pgen (int size, SFData * sf, FILE * fd)
{
	...
	{			/* traverse preset's zones */
	  ...
	  while (p3)
	    {			/* load zone's generators */
	      ...
	      if ((size -= SFGENSIZE) < 0)
		return (gerr (ErrCorr,
		    (char*) _("Preset generator chunk size mismatch")));
	  ...
	  if (level == 3)
		  ...
		  if (*hz != p2)
		    {
		      void* save = p2->data;
		      FLUID_LOG (FLUID_WARN,
			(char*) _("Preset \"%s\": Global zone is not first zone"),
		...
		{		/* previous global zone exists, discard */
		  FLUID_LOG (FLUID_WARN,
		    (char*) _("Preset \"%s\": Discarding invalid global zone"),
		    ((SFPreset *) (p->data))->name);
      ...
      if (discarded)
	FLUID_LOG(FLUID_WARN,
	  (char*) _("Preset \"%s\": Some invalid generators were discarded"),
	  ((SFPreset *) (p->data))->name);
      p = fluid_list_next (p);
    }
  ...
  size -= SFGENSIZE;
  if (size != 0)
    return (gerr (ErrCorr, (char*) _("Preset generator chunk size mismatch")));
...
load_ihdr (int size, SFData * sf, FILE * fd)
{
  ...
  if (size % SFIHDRSIZE || size == 0)	/* chunk size is valid? */
    return (gerr (ErrCorr, (char*) _("Instrument header has invalid size")));

  size = size / SFIHDRSIZE - 1;
  if (size == 0)
    {				/* at least one preset + term record */
      FLUID_LOG (FLUID_WARN, (char*) _("File contains no instruments"));
      FSKIP (SFIHDRSIZE, fd);
      return (OK);
    }
  ...
  for (i = 0; i < size; i++)
	...
	{			/* not first instrument? */
	  if (zndx < pzndx)
	    return (gerr (ErrCorr,
		(char*) _("Instrument header indices not monotonic")));
	  i2 = zndx - pzndx;
	  while (i2--)
	    pr->zone = fluid_list_prepend (pr->zone, NULL);
	}
      else if (zndx > 0)	/* 1st inst, warn if ofs >0 */
	FLUID_LOG (FLUID_WARN, (char*) _("%d instrument zones not referenced, discarding"),
	  zndx);
      pzndx = zndx;
  ...
  if (zndx < pzndx)
    return (gerr (ErrCorr, (char*) _("Instrument header indices not monotonic")));
  i2 = zndx - pzndx;
  while (i2--)
    pr->zone = fluid_list_prepend (pr->zone, NULL);
...
load_ibag (int size, SFData * sf, FILE * fd)
{
  ...
  if (size % SFBAGSIZE || size == 0)	/* size is multiple of SFBAGSIZE? */
    return (gerr (ErrCorr, (char*) _("Instrument bag chunk size is invalid")));
  ...
  while (p)
	...
	{			/* load this inst's zones */
	  if ((size -= SFBAGSIZE) < 0)
	    return (gerr (ErrCorr, (char*) _("Instrument bag chunk size mismatch")));
	  z = FLUID_NEW (SFZone);
	  p2->data = z;
	  z->gen = NULL;	/* In case of failure, */
	  z->mod = NULL;	/* sfont_close can clean up */
	  READWU (genndx, fd);	/* READW = possible read failure */
	  READWU (modndx, fd);
	  z->instsamp = NULL;
	  ...
	  if (pz)
	    {			/* if not first zone */
	      if (genndx < pgenndx)
		return (gerr (ErrCorr,
		    (char*) _("Instrument generator indices not monotonic")));
	      if (modndx < pmodndx)
		return (gerr (ErrCorr,
		    (char*) _("Instrument modulator indices not monotonic")));
	      i = genndx - pgenndx;
	      while (i--)
		pz->gen = fluid_list_prepend (pz->gen, NULL);
	      i = modndx - pmodndx;
  ...
  size -= SFBAGSIZE;
  if (size != 0)
    return (gerr (ErrCorr, (char*) _("Instrument chunk size mismatch")));
  ...
  if (!pz)
    {				/* in case that all are no zoners */
      if (genndx > 0)
	FLUID_LOG (FLUID_WARN,
	  (char*) _("No instrument generators and terminal index not 0"));
      if (modndx > 0)
	FLUID_LOG (FLUID_WARN,
	  (char*) _("No instrument modulators and terminal index not 0"));
      return (OK);
    }

  if (genndx < pgenndx)
    return (gerr (ErrCorr, (char*) _("Instrument generator indices not monotonic")));
  if (modndx < pmodndx)
    return (gerr (ErrCorr, (char*) _("Instrument modulator indices not monotonic")));
  i = genndx - pgenndx;
  while (i--)
    pz->gen = fluid_list_prepend (pz->gen, NULL);
  i = modndx - pmodndx;
...
load_imod (int size, SFData * sf, FILE * fd)
{
	...
	{			/* traverse this inst's zones */
	  ...
	  while (p3)
	    {			/* load zone's modulators */
	      if ((size -= SFMODSIZE) < 0)
		return (gerr (ErrCorr,
		    (char*) _("Instrument modulator chunk size mismatch")));
	      m = FLUID_NEW (SFMod);
	      p3->data = m;
	      READWU (m->src, fd);
	      READWU (m->dest, fd);
  ...
  size -= SFMODSIZE;
  if (size != 0)
    return (gerr (ErrCorr, (char*) _("Instrument modulator chunk size mismatch")));
  FSKIP (SFMODSIZE, fd);	/* terminal mod */
	...
	{			/* traverse this instrument's zones */
	  ...
	  while (p3)
	    {			/* load zone's generators */
	      ...
	      drop = FALSE;
	      if ((size -= SFGENSIZE) < 0)
		return (gerr (ErrCorr, (char*) _("IGEN chunk size mismatch")));
	  ...
	  if (level == 3)
		  ...
		  if (*hz != p2)
		    {
		      void* save = p2->data;
		      FLUID_LOG (FLUID_WARN,
			(char*) _("Instrument \"%s\": Global zone is not first zone"),
			((SFPreset *) (p->data))->name);
		      SLADVREM (*hz, p2);
		      *hz = fluid_list_prepend (*hz, save);
		      continue;
		...
		{		/* previous global zone exists, discard */
		  FLUID_LOG (FLUID_WARN,
		    (char*) _("Instrument \"%s\": Discarding invalid global zone"),
		    ((SFInst *) (p->data))->name);
		  sfont_zone_delete (sf, hz, (SFZone *) (p2->data));
		}
	    }
	  ...
	  while (p3)
	    {			/* Kill any zones following a sample */
	      ...
	      if ((size -= SFGENSIZE) < 0)
		return (gerr (ErrCorr,
		    (char*) _("Instrument generator chunk size mismatch")));
	      FSKIP (SFGENSIZE, fd);
	      SLADVREM (z->gen, p3);
	    }
      ...
      if (discarded)
	FLUID_LOG(FLUID_WARN,
	  (char*) _("Instrument \"%s\": Some invalid generators were discarded"),
	  ((SFInst *) (p->data))->name);
      p = fluid_list_next (p);
    }
...
load_shdr (unsigned int size, SFData * sf, FILE * fd)
{
  ...
  if (size % SFSHDRSIZE || size == 0)	/* size is multiple of SHDR size? */
    return (gerr (ErrCorr, (char*) _("Sample header has invalid size")));

  size = size / SFSHDRSIZE - 1;
  if (size == 0)
    {				/* at least one sample + term record? */
      FLUID_LOG (FLUID_WARN, (char*) _("File contains no samples"));
      FSKIP (SFSHDRSIZE, fd);
      return (OK);
    }
  ...
  for (i = 0; i < size; i++)
    {
      ...
      READSTR (&p->name, fd);
      READDU (p->start, fd);
      READDU (p->end, fd);	/* - end, loopstart and loopend */
      READDU (p->loopstart, fd);	/* - will be checked and turned into */
      READDU (p->loopend, fd);	/* - offsets in fixup_sample() */
      READDU (p->samplerate, fd);
      READB (p->origpitch, fd);
      READB (p->pitchadj, fd);
      FSKIPW (fd);		/* skip sample link */
      READWU (p->sampletype, fd);
      p->samfile = 0;
    }
	...
	{			/* traverse this preset's zones */
	  ...
	  if ((i = GPOINTER_TO_INT (z->instsamp)))
	    {			/* load instrument # */
	      ...
	      if (!p3)
		return (gerr (ErrCorr,
		    (char*) _("Preset %03d %03d: Invalid instrument reference"),
	...
	{			/* traverse instrument's zones */
      ...
      if ((i = GPOINTER_TO_INT (z->instsamp)))
	    {			/* load sample # */
	      ...
	      if (!p3)
		return (gerr (ErrCorr,
		    (char*) _((char*)"Instrument \"%s\": Invalid sample reference"),
		    ((SFInst *) (p->data))->name));
	      z->instsamp = p3;
	    }
	  p2 = fluid_list_next (p2);
...
fixup_sample (SFData * sf)
{
  ...
  while (p)
	...
	{
	  FLUID_LOG (FLUID_WARN, (char*) _("Sample '%s' start/end file positions are invalid,"
...
{
  if (fread (buf, (size_t) count, (size_t) 1, fd) != 1)
    {				/* size_t = count, nmemb = 1 */
      if (feof (fd))
	gerr (ErrEof, (char*) _("EOF while attemping to read %d bytes"), count);
      else
	FLUID_LOG (FLUID_ERR, (char*) _("File read failed"));
      return (FAIL);
    }
  return (OK);
}
...
safe_fseek (FILE * fd, long ofs, int whence)
{
  if (fseek (fd, ofs, whence) == -1) {
    FLUID_LOG (FLUID_ERR, (char*) _("File seek failed with offset = %ld and whence = %d"), ofs, whence);
    return (FAIL);
  }
  return (OK);
}
