...
new_fluid_chorus(fluid_real_t sample_rate)
{
  ...
  if (chorus == NULL) {
    fluid_log(FLUID_PANIC, (char*) "chorus: Out of memory");
    return NULL;
  }
  ...
  chorus->lookup_tab = FLUID_ARRAY(int, (size_t) (chorus->sample_rate / MIN_SPEED_HZ));
...
fluid_chorus_update(fluid_chorus_t* chorus)
{
  ...
  if (chorus->new_number_blocks < 0) {
    fluid_log(FLUID_WARN, (char*) "chorus: number blocks must be >=0! Setting value to 0.");
    chorus->new_number_blocks = 0;
  } else if (chorus->new_number_blocks > MAX_CHORUS) {
    fluid_log(FLUID_WARN, (char*) "chorus: number blocks larger than max. allowed! Setting value to %d.",
	     MAX_CHORUS);
    chorus->new_number_blocks = MAX_CHORUS;
  };

  if (chorus->new_speed_Hz < MIN_SPEED_HZ) {
    fluid_log(FLUID_WARN, (char*) "chorus: speed is too low (min %f)! Setting value to min.",
	     (double) MIN_SPEED_HZ);
    chorus->new_speed_Hz = (fluid_real_t) MIN_SPEED_HZ;
  } else if (chorus->new_speed_Hz > MAX_SPEED_HZ) {
    fluid_log(FLUID_WARN, (char*) "chorus: speed must be below %f Hz! Setting value to max.",
	     (double) MAX_SPEED_HZ);
    chorus->new_speed_Hz = MAX_SPEED_HZ;
  }
  if (chorus->new_depth_ms < 0.0) {
    fluid_log(FLUID_WARN, (char*) "chorus: depth must be positive! Setting value to 0.");
    chorus->new_depth_ms = 0.0;
  }
  /* Depth: Check for too high value through modulation_depth_samples. */

  if (chorus->new_level < 0.0) {
    fluid_log(FLUID_WARN, (char*) "chorus: level must be positive! Setting value to 0.");
    chorus->new_level = 0.0;
  } else if (chorus->new_level > 10) {
    fluid_log(FLUID_WARN, (char*) "chorus: level must be < 10. A reasonable level is << 1! "
	     "Setting it to 0.1.");
    chorus->new_level = (fluid_real_t) 0.1;
  }

  /* The modulating LFO goes through a full period every x samples: */
  chorus->modulation_period_samples = (long) (chorus->sample_rate / chorus->new_speed_Hz);
  ...
  if (modulation_depth_samples > MAX_SAMPLES) {
    fluid_log(FLUID_WARN, (char*) "chorus: Too high depth. Setting it to max (%d).", MAX_SAMPLES);
  ...
  if (chorus->type == FLUID_CHORUS_MOD_SINE) {
    ...
    fluid_log(FLUID_WARN, (char*) "chorus: Unknown modulation type. Using sinewave.");
