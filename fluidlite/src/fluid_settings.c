...
static int fluid_settings_tokenize(const char* s, char *buf, char** ptr)
{
  ...
  {
    FLUID_LOG(FLUID_ERR, (char*) "Setting variable name exceeded max length of %d chars",
	      MAX_SETTINGS_LABEL);
    return 0;
  }
  ...
  while ((tok = fluid_strtok (&tokstr, (char*) ".")))
  {
    if (n > MAX_SETTINGS_TOKENS)
    {
      FLUID_LOG(FLUID_ERR, (char*) "Setting variable name exceeded max token count of %d",
		MAX_SETTINGS_TOKENS);
      return 0;
    }
			     ...
			     void* value, int type)
{
  ...
  for (n = 0; n < num; n++) {
	...
	FLUID_LOG(FLUID_WARN, (char*) "'%s' is not a node", name[n]);
	return 0;
      }
			       ...
			       fluid_str_update_t fun, void* data)
{
  ...
  if (!fluid_settings_get(settings, tokens, ntokens, &value, &type)) {
    ...
    if (type == FLUID_STR_TYPE) {
      ...
      FLUID_LOG(FLUID_WARN, (char*) "Type mismatch on setting '%s'", name);
      return 1;
    }
  }
}
...
int fluid_settings_setstr(fluid_settings_t* settings, const char* name, const char* str)
{
  ...
  if (fluid_settings_get(settings, tokens, ntokens, &value, &type)) {
    ...
    /* insert a new setting */
    fluid_str_setting_t* s;
    s = new_fluid_str_setting(str, NULL, 0, NULL, NULL);
    return fluid_settings_set(settings, tokens, ntokens, s, FLUID_STR_TYPE);
  }
}
...
int fluid_settings_setnum(fluid_settings_t* settings, const char* name, double val)
{
  ...
  if (fluid_settings_get(settings, tokens, ntokens, &value, &type)) {
    ...
    /* insert a new setting */
    fluid_num_setting_t* s;
    s = new_fluid_num_setting(-1e10, 1e10, 0.0f, 0, NULL, NULL);
    s->value = val;
    return fluid_settings_set(settings, tokens, ntokens, s, FLUID_NUM_TYPE);
  }
}
...
int fluid_settings_setint(fluid_settings_t* settings, const char* name, int val)
{
  ...
  if (fluid_settings_get(settings, tokens, ntokens, &value, &type)) {
    ...
    /* insert a new setting */
    fluid_int_setting_t* s;
    s = new_fluid_int_setting(INT_MIN, INT_MAX, 0, 0, NULL, NULL);
    s->value = val;
    return fluid_settings_set(settings, tokens, ntokens, s, FLUID_INT_TYPE);
  }
}
