...
int fluid_rampreset_add_sample(fluid_rampreset_t* preset, fluid_sample_t* sample, int lokey, int hikey)
{
	...
	if (preset->zone == NULL) {
		fluid_preset_zone_t* zone;
		zone = new_fluid_preset_zone((char*) "");
		if (zone == NULL) {
			return FLUID_FAILED;
		}
	...
	/* add an instrument zone for each sample */
	{
		...
		fluid_inst_zone_t* izone = new_fluid_inst_zone((char*) "");
		if (izone == NULL) {
			return FLUID_FAILED;
		}
	...
	if (fluid_inst_zone_inside_range(inst_zone, key, vel) && (sample != NULL)) {
	  ...
	  for (i = 0; i < GEN_LAST; i++) {
	    ...
	    if (inst_zone->gen[i].flags){
	      fluid_voice_gen_set(voice, i, (fluid_real_t) inst_zone->gen[i].val);

	    } else if (global_inst_zone != NULL && global_inst_zone->gen[i].flags){
	      fluid_voice_gen_set(voice, i, (fluid_real_t) global_inst_zone->gen[i].val);
...
fluid_sample_set_sound_data(fluid_sample_t* sample, short *data, unsigned int nbframes, short copy_data, int rootkey)
{
  ...
  if (sample->data != NULL) {
	...
	if (copy_data) {
	  ...
	  sample->data = (short*) FLUID_MALLOC(storedNbFrames*2 + 4*SAMPLE_LOOP_MARGIN);
	  if (sample->data == NULL) {
	    FLUID_LOG(FLUID_ERR, (char*) "Out of memory");
	    return FLUID_FAILED;
	  }
