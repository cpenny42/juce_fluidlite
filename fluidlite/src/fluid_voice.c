		 ...
		 unsigned int start_time, fluid_real_t gain)
{
  ...
  voice->id = id;
  voice->chan = (unsigned char) fluid_channel_get_num(channel);
  ...
  if (voice->synth_gain < 0.0000001){
    voice->synth_gain = (fluid_real_t) 0.0000001;
  }
  ...
  voice->amplitude_that_reaches_noise_floor_nonloop = (fluid_real_t) (FLUID_NOISE_FLOOR / voice->synth_gain);
  voice->amplitude_that_reaches_noise_floor_loop = (fluid_real_t) (FLUID_NOISE_FLOOR / voice->synth_gain);
				 ...
				 int gen_key2base, int is_decay)
{
   ...
   */
  timecents = (fluid_real_t) (_GEN(voice, gen_base) + _GEN(voice, gen_key2base) * (60.0 - voice->key));
...
fluid_voice_update_param(fluid_voice_t* voice, int gen)
{
  ...
  case GEN_OVERRIDEROOTKEY:
    ...
    if (voice->gen[GEN_OVERRIDEROOTKEY].val > -1) {   //FIXME: use flag instead of -1
      voice->root_pitch = (fluid_real_t) (voice->gen[GEN_OVERRIDEROOTKEY].val * 100.0f
	- (fluid_real_t) voice->sample->pitchadj);
    } else {
  ...
  case GEN_STARTADDRCOARSEOFS:        /* SF2.01 section 8.1.3 # 4 */
    if (voice->sample != NULL) {
      voice->start = (int) (voice->sample->start
			     + (unsigned int) _GEN(voice, GEN_STARTADDROFS)
			     + 32768 * (unsigned int) _GEN(voice, GEN_STARTADDRCOARSEOFS));
  ...
  case GEN_ENDADDRCOARSEOFS:           /* SF2.01 section 8.1.3 # 12 */
    if (voice->sample != NULL) {
      voice->end = (int) (voice->sample->end
			   + (unsigned int) _GEN(voice, GEN_ENDADDROFS)
			   + 32768 * (unsigned int) _GEN(voice, GEN_ENDADDRCOARSEOFS));
      voice->check_sample_sanity_flag = FLUID_SAMPLESANITY_CHECK;
    }
    break;
  ...
  case GEN_STARTLOOPADDRCOARSEOFS:     /* SF2.01 section 8.1.3 # 45 */
    if (voice->sample != NULL) {
      voice->loopstart = (int) (voice->sample->loopstart
				  + (unsigned int) _GEN(voice, GEN_STARTLOOPADDROFS)
				  + 32768 * (unsigned int) _GEN(voice, GEN_STARTLOOPADDRCOARSEOFS));
      voice->check_sample_sanity_flag = FLUID_SAMPLESANITY_CHECK;
    }
    break;
  ...
  case GEN_ENDLOOPADDRCOARSEOFS:       /* SF2.01 section 8.1.3 # 50 */
    if (voice->sample != NULL) {
      voice->loopend = (int) (voice->sample->loopend
				+ (unsigned int) _GEN(voice, GEN_ENDLOOPADDROFS)
				+ 32768 * (unsigned int) _GEN(voice, GEN_ENDLOOPADDRCOARSEOFS));
      voice->check_sample_sanity_flag = FLUID_SAMPLESANITY_CHECK;
    }
    break;
...
#define NUM_BUFFERS_RELEASE(_v) (unsigned int) (voice->output_rate * fluid_tc2sec_release(_v) / FLUID_BUFSIZE)
  ...
  case GEN_KEYTOVOLENVHOLD:            /* SF2.01 section 8.1.3 # 39 */
    count = (unsigned int) calculate_hold_decay_buffers(voice, GEN_VOLENVHOLD, GEN_KEYTOVOLENVHOLD, 0); /* 0 means: hold */
  ...
  case GEN_KEYTOVOLENVDECAY:          /* SF2.01 section 8.1.3 # 40 */
    ...
    count = (unsigned int) calculate_hold_decay_buffers(voice, GEN_VOLENVDECAY, GEN_KEYTOVOLENVDECAY, 1); /* 1 for decay */
  ...
  case GEN_KEYTOMODENVHOLD:          /* SF2.01 section 8.1.3 # 31 */
    count = (unsigned int) calculate_hold_decay_buffers(voice, GEN_MODENVHOLD, GEN_KEYTOMODENVHOLD, 0); /* 1 means: hold */
  ...
  case GEN_KEYTOMODENVDECAY:                              /* SF 2.01 section 8.1.3 # 32 */
    count = (unsigned int) calculate_hold_decay_buffers(voice, GEN_MODENVDECAY, GEN_KEYTOMODENVDECAY, 1); /* 1 for decay */
...
fluid_voice_noteoff(fluid_voice_t* voice)
{
  ...
  if (voice->channel && fluid_channel_sustained(voice->channel)) {
	...
	fluid_real_t lfo = voice->modlfo_val * -voice->modlfo_to_vol;
        fluid_real_t amp = (fluid_real_t) (voice->volenv_val * pow (10.0, lfo / -200));
...
fluid_voice_add_mod(fluid_voice_t* voice, fluid_mod_t* mod, int mode)
{
  ...
  if (((mod->flags1 & FLUID_MOD_CC) == 0)
    ...
    FLUID_LOG(FLUID_WARN, (char*) "Ignoring invalid controller, using non-CC source %i.", mod->src1);
    return;
  }
...
void fluid_voice_check_sample_sanity(fluid_voice_t* voice)
{
...
#if 0
    printf((char*) "Sample from %i to %i\n",voice->sample->start, voice->sample->end);
    printf((char*) "Sample loop from %i %i\n",voice->sample->loopstart, voice->sample->loopend);
    printf((char*) "Playback from %i to %i\n", voice->start, voice->end);
    printf((char*) "Playback loop from %i to %i\n",voice->loopstart, voice->loopend);
#endif
	...
	if (voice->sample->amplitude_that_reaches_noise_floor_is_valid){
	  voice->amplitude_that_reaches_noise_floor_loop = (fluid_real_t) (voice->sample->amplitude_that_reaches_noise_floor / voice->synth_gain);
	} else {
	  /* Worst case */
	...
	|| (_SAMPLEMODE(voice) == FLUID_LOOP_DURING_RELEASE)) {
       ...
       * actions required.
       */
      int index_in_sample = (int) fluid_phase_index(voice->phase);
...
#if 0
    printf((char*) "Sane? playback loop from %i to %i\n", voice->loopstart, voice->loopend);
#endif
}
...
#if 0
    printf((char*) "Sample peak detection: factor %f\n", (double)result);
#endif
  };
  return FLUID_OK;
}
