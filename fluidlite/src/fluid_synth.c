...
{
  fluid_settings_register_str(settings, (char*) "synth.verbose", (char*) "no", 0, NULL, NULL);
  fluid_settings_register_str(settings, (char*) "synth.dump", (char*) "no", 0, NULL, NULL);
  fluid_settings_register_str(settings, (char*) "synth.reverb.active", (char*) "yes", 0, NULL, NULL);
  fluid_settings_register_str(settings, (char*) "synth.chorus.active", (char*) "yes", 0, NULL, NULL);
  fluid_settings_register_str(settings, (char*) "synth.ladspa.active", (char*) "no", 0, NULL, NULL);
  fluid_settings_register_str(settings, (char*) "midi.portname", (char*) "", 0, NULL, NULL);
  fluid_settings_register_str(settings, (char*) "synth.drums-channel.active", (char*) "yes", 0, NULL, NULL);

  fluid_settings_register_int(settings, (char*) "synth.polyphony",
			     256, 16, 4096, 0, NULL, NULL);
  fluid_settings_register_int(settings, (char*) "synth.midi-channels",
			     16, 16, 256, 0, NULL, NULL);
  fluid_settings_register_num(settings, (char*) "synth.gain",
			     0.2f, 0.0f, 10.0f,
			     0, NULL, NULL);
  fluid_settings_register_int(settings, (char*) "synth.audio-channels",
			     1, 1, 256, 0, NULL, NULL);
  fluid_settings_register_int(settings, (char*) "synth.audio-groups",
			     1, 1, 256, 0, NULL, NULL);
  fluid_settings_register_int(settings, (char*) "synth.effects-channels",
			     2, 2, 2, 0, NULL, NULL);
  fluid_settings_register_num(settings, (char*) "synth.sample-rate",
			     44100.0f, 22050.0f, 96000.0f,
			     0, NULL, NULL);
  fluid_settings_register_int(settings, (char*) "synth.min-note-length", 10, 0, 65535, 0, NULL, NULL);
}
...
new_fluid_synth(fluid_settings_t *settings)
{
  ...
  if (synth == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Out of memory");
    return NULL;
  }
  ...
  synth->with_reverb = (char) fluid_settings_str_equal(settings, (char*) "synth.reverb.active", (char*) "yes");
  synth->with_chorus = (char) fluid_settings_str_equal(settings, (char*) "synth.chorus.active", (char*) "yes");
  synth->verbose = (char) fluid_settings_str_equal(settings, (char*) "synth.verbose", (char*) "yes");
  synth->dump = (char) fluid_settings_str_equal(settings, (char*) "synth.dump", (char*) "yes");

  fluid_settings_getint(settings, (char*) "synth.polyphony", &synth->polyphony);
  fluid_settings_getnum(settings, (char*) "synth.sample-rate", &synth->sample_rate);
  fluid_settings_getint(settings, (char*) "synth.midi-channels", &synth->midi_channels);
  fluid_settings_getint(settings, (char*) "synth.audio-channels", &synth->audio_channels);
  fluid_settings_getint(settings, (char*) "synth.audio-groups", &synth->audio_groups);
  fluid_settings_getint(settings, (char*) "synth.effects-channels", &synth->effects_channels);
  fluid_settings_getnum(settings, (char*) "synth.gain", &synth->gain);
  fluid_settings_getint(settings, (char*) "synth.min-note-length", &i);
  ...
  if (synth->midi_channels % 16 != 0) {
    ...
    fluid_settings_setint(settings, "(char*) synth.midi-channels", synth->midi_channels);
    FLUID_LOG(FLUID_WARN, (char*) "Requested number of MIDI channels is not a multiple of 16. "
	     "I'll increase the number of channels to the next multiple.");
  }

  if (synth->audio_channels < 1) {
    FLUID_LOG(FLUID_WARN, (char*) "Requested number of audio channels is smaller than 1. "
	     "Changing this setting to 1.");
    synth->audio_channels = 1;
  } else if (synth->audio_channels > 128) {
    FLUID_LOG(FLUID_WARN, (char*) "Requested number of audio channels is too big (%d). "
	     "Limiting this setting to 128.", synth->audio_channels);
    synth->audio_channels = 128;
  }

  if (synth->audio_groups < 1) {
    FLUID_LOG(FLUID_WARN, (char*) "Requested number of audio groups is smaller than 1. "
	     "Changing this setting to 1.");
    synth->audio_groups = 1;
  } else if (synth->audio_groups > 128) {
    FLUID_LOG(FLUID_WARN, (char*) "Requested number of audio groups is too big (%d). "
	     "Limiting this setting to 128.", synth->audio_groups);
    synth->audio_groups = 128;
  }

  if (synth->effects_channels != 2) {
    FLUID_LOG(FLUID_WARN, (char*) "Invalid number of effects channels (%d)."
  ...
  if (loader == NULL) {
    FLUID_LOG(FLUID_WARN, (char*) "Failed to create the default SoundFont loader");
  } else {
    fluid_synth_add_sfloader(synth, loader);
  }

  /* allocate all channel objects */
  synth->channel = FLUID_ARRAY(fluid_channel_t*, (size_t) synth->midi_channels);
  ...
  synth->voice = FLUID_ARRAY(fluid_voice_t*, (size_t) synth->nvoice);
  if (synth->voice == NULL) {
    goto error_recovery;
  }
  ...
  synth->left_buf = FLUID_ARRAY(fluid_real_t*, (size_t) synth->nbuf);
  synth->right_buf = FLUID_ARRAY(fluid_real_t*, (size_t) synth->nbuf);
  ...
  FLUID_MEMSET(synth->left_buf, 0, (size_t) synth->nbuf * sizeof(fluid_real_t*));
  FLUID_MEMSET(synth->right_buf, 0, (size_t) synth->nbuf * sizeof(fluid_real_t*));
  ...
  synth->fx_left_buf = FLUID_ARRAY(fluid_real_t*, (size_t) synth->effects_channels);
  synth->fx_right_buf = FLUID_ARRAY(fluid_real_t*, (size_t) synth->effects_channels);
  ...
  if(fluid_settings_str_equal(settings, (char*) "synth.drums-channel.active", (char*) "yes"))
...
//  int r = FLUID_FAILED;
  ...
  if ((chan < 0) || (chan >= synth->midi_channels)) {
    FLUID_LOG(FLUID_WARN, (char*) "Channel out of range");
    return FLUID_FAILED;
  }
  ...
  if (channel->preset == NULL) {
    if (synth->verbose) {
      FLUID_LOG(FLUID_INFO, (char*) "noteon\t%d\t%d\t%d\t%05d\t%.3f\t\t%.3f\t%d\t%s",
	       chan, key, vel, 0,
	       (float) synth->ticks / 44100.0f,
	       0.0f, 0, (char*) "channel has no preset");
    }
    return FLUID_FAILED;
  }
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  for (i = 0; i < synth->polyphony; i++) {
    ...
    FLUID_LOG(FLUID_INFO, (char*) "noteoff\t%d\t%d\t%d\t%05d\t%.3f\t\t%.3f\t%d",
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  if ((num < 0) || (num >= 128)) {
    FLUID_LOG(FLUID_WARN, (char*) "Ctrl out of range");
    return FLUID_FAILED;
  }
  if ((val < 0) || (val >= 128)) {
    FLUID_LOG(FLUID_WARN, (char*) "Value out of range");
    return FLUID_FAILED;
  }

  if (synth->verbose) {
    FLUID_LOG(FLUID_INFO, (char*) "cc\t%d\t%d\t%d", chan, num, val);
  }
    ...
    case MIDI_SYSEX_TUNING_BULK_DUMP_REQ_BANK:
      ...
      *resptr++ = (char) prog;
      ...
      {  /* NOTE: Checksum is not as straight forward as the bank based messages */
        chksum = (uint8) (MIDI_SYSEX_UNIV_NON_REALTIME ^ MIDI_SYSEX_MIDI_TUNING_ID
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  if (synth->verbose) {
    FLUID_LOG(FLUID_INFO, (char*) "channelpressure\t%d\t%d", chan, val);
  }
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  if (synth->verbose) {
    FLUID_LOG(FLUID_INFO, (char*) "pitchb\t%d\t%d", chan, val);
  }
...
fluid_synth_pitch_wheel_sens(fluid_synth_t* synth, int chan, int val)
{
  ...
  if (synth->verbose) {
    FLUID_LOG(FLUID_INFO, (char*) "pitchsens\t%d\t%d", chan, val);
  }
...
//  fluid_list_t* list = synth->sfont;
  ...
  if (sfont != NULL) {
    offset = fluid_synth_get_bank_offset(synth, (int) sfontnum);
    preset = fluid_sfont_get_preset(sfont, (unsigned int) (banknum - (unsigned int) offset), (unsigned int) prognum);
    if (preset != NULL) {
      return preset;
    }
  }
			...
			unsigned int banknum, unsigned int prognum)
{
  ...
  if (sfont != NULL) {
    offset = fluid_synth_get_bank_offset(synth, (int) fluid_sfont_get_id(sfont));
    preset = fluid_sfont_get_preset(sfont, banknum - (unsigned int) offset, prognum);
    if (preset != NULL) {
      return preset;
    }
  }
...
fluid_synth_program_change(fluid_synth_t* synth, int chan, int prognum)
{
  ...
  {
    FLUID_LOG(FLUID_ERR, (char*) "Index out of range (chan=%d, prog=%d)", chan, prognum);
    return FLUID_FAILED;
  }
  ...
  if (synth->verbose)
    FLUID_LOG(FLUID_INFO, (char*) "prog\t%d\t%d\t%d", chan, banknum, prognum);

  preset = fluid_synth_find_preset(synth, (unsigned int) banknum, (unsigned int) prognum); //if (channel->channum == 9) fluid_synth_find_preset(synth, DRUM_INST_BANK, prognum);

  /* Fallback to another preset if not found */
  if (!preset)
  {
    subst_bank = (int) banknum;
    subst_prog = prognum;
    ...
    if (banknum != DRUM_INST_BANK)
    {
      ...
      preset = fluid_synth_find_preset(synth, 0, (unsigned int) prognum);
    ...
    if (preset)
      FLUID_LOG(FLUID_WARN, (char*) "Instrument not found on channel %d [bank=%d prog=%d], substituted [bank=%d prog=%d]",
		       ...
		       unsigned int* sfont_id, unsigned int* bank_num, unsigned int* preset_num)
{
  ...
  if ((chan >= 0) && (chan < synth->midi_channels)) {
    ...
    *preset_num = (unsigned int) fluid_channel_get_prognum(channel);
    return FLUID_OK;
  }
  return FLUID_FAILED;
}
			      ...
			      unsigned int preset_num)
{
  ...
  if ((chan < 0) || (chan >= synth->midi_channels)) {
    FLUID_LOG(FLUID_ERR, (char*) "Channel number out of range (chan=%d)", chan);
    return FLUID_FAILED;
  }
  channel = synth->channel[chan];
  ...
  if (preset == NULL) {
    FLUID_LOG(FLUID_ERR,
	     (char*) "There is no preset with bank number %d and preset number %d in SoundFont %d",
  ...
  fluid_channel_set_prognum(channel, (int) preset_num);
				...
				unsigned int preset_num)
{
  ...
  sfont = fluid_synth_get_sfont_by_name(synth, sfont_name);
  if (sfont == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Could not find SoundFont %s", sfont_name);
    return FLUID_FAILED;
  }

  offset = fluid_synth_get_bank_offset(synth, (int) fluid_sfont_get_id(sfont));
  preset = fluid_sfont_get_preset(sfont, bank_num - (unsigned int) offset, preset_num);
  if (preset == NULL) {
    FLUID_LOG(FLUID_ERR,
	      (char*) "There is no preset with bank number %d and preset number %d in SoundFont %s",
  ...
  for (chan = 0; chan < synth->midi_channels; chan++) {
    ...
    fluid_channel_set_preset(channel,
			    fluid_synth_get_preset(synth,
						  ...
						  (unsigned int) fluid_channel_get_banknum(channel),
						  (unsigned int) fluid_channel_get_prognum(channel)));
  }
}
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  fluid_revmodel_setroomsize(synth->reverb, (fluid_real_t) roomsize);
  fluid_revmodel_setdamp(synth->reverb, (fluid_real_t) damping);
  fluid_revmodel_setwidth(synth->reverb, (fluid_real_t) width);
  fluid_revmodel_setlevel(synth->reverb, (fluid_real_t) level);
}
       ...
       float** fx_left, float** fx_right)
{
  ...
  if (synth->cur < FLUID_BUFSIZE) {
    ...
    bytes = (int) ((size_t)num * sizeof(float));

    for (i = 0; i < synth->audio_channels; i++) {
      FLUID_MEMCPY(left[i], left_in[i] + synth->cur, (size_t) bytes);
      FLUID_MEMCPY(right[i], right_in[i] + synth->cur, (size_t) bytes);
    }
    count += num;
  ...
  while (count < len) {
    ...
    for (i = 0; i < synth->audio_channels; i++) {
      FLUID_MEMCPY(left[i] + count, left_in[i], (size_t) bytes);
      FLUID_MEMCPY(right[i] + count, right_in[i], (size_t) bytes);
    }
		       ...
		       int nout, float** out)
{
  if (nout==2) {
    ...
    float **left, **right;
    int i;
    left = FLUID_ARRAY(float*, (size_t)nout/2);
    right = FLUID_ARRAY(float*, (size_t)nout/2);
    for(i=0; i<nout/2; i++) {
      left[i] = out[2*i];
      right[i] = out[2*i+1];
    }
...
/*   fluid_mutex_lock(synth->busy); /\* Here comes the audio thread. Lock the synth. *\/ */
  ...
  for (i = 0; i < synth->nbuf; i++) {
    FLUID_MEMSET(synth->left_buf[i], 0, (size_t) byte_size);
    FLUID_MEMSET(synth->right_buf[i], 0, (size_t) byte_size);
  }

  for (i = 0; i < synth->effects_channels; i++) {
    FLUID_MEMSET(synth->fx_left_buf[i], 0, (size_t) byte_size);
    FLUID_MEMSET(synth->fx_right_buf[i], 0, (size_t) byte_size);
  }
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  if (voice == NULL) {
    FLUID_LOG(FLUID_WARN, (char*) "Failed to allocate a synthesis process. (chan=%d,key=%d)", chan, key);
    return NULL;
  }
  ...
  if (synth->verbose) {
    ...
    FLUID_LOG(FLUID_INFO, (char*) "noteon\t%d\t%d\t%d\t%05d\t%.3f\t\t%.3f\t%d",
  ...
  if (fluid_voice_init(voice, sample, channel, key, vel,
		       synth->storeid, synth->ticks, (fluid_real_t)synth->gain) != FLUID_OK) {
    FLUID_LOG(FLUID_WARN, (char*) "Failed to initialize voice");
    return NULL;
  }
...
void fluid_synth_kill_by_exclusive_class(fluid_synth_t* synth, fluid_voice_t* new_voice)
{
  ...
  */
  ...
  int i;
  int excl_class = (int) _GEN(new_voice,GEN_EXCLUSIVECLASS);
...
fluid_synth_sfload(fluid_synth_t* synth, const char* filename, int reset_presets)
{
  ...
  if (filename == NULL) {
    FLUID_LOG(FLUID_ERR, (char*) "Invalid filename");
    return FLUID_FAILED;
  }
  ...
  FLUID_LOG(FLUID_ERR, (char*) "Failed to load SoundFont \"%s\"", filename);
  return -1;
}
...
fluid_synth_sfunload(fluid_synth_t* synth, unsigned int id, int reset_presets)
{
  ...
  if (!sfont) {
    FLUID_LOG(FLUID_ERR, (char*) "No SoundFont with id = %d", id);
    return FLUID_FAILED;
  }
  ...
  if (r == 0) {
    FLUID_LOG(FLUID_DBG, (char*) "Unloaded SoundFont");
  }
#endif
  }
...
{
	int sfont_id = (int) fluid_sfont_get_id(sfont);
...
{
  return (fluid_sfont_t*) fluid_list_get(fluid_list_nth(synth->sfont, (int) num));
}
...
int fluid_synth_set_interp_method(fluid_synth_t* synth, int chan, int interp_method){
  ...
  for (i = 0; i < synth->midi_channels; i++) {
    if (synth->channel[i] == NULL){
      FLUID_LOG(FLUID_ERR, (char*) "Channels don't exist (yet)!");
      return FLUID_FAILED;
    };
...
{
  if ((bank < 0) || (bank >= 128)) {
    FLUID_LOG(FLUID_WARN, (char*) "Bank number out of range");
    return NULL;
  }
  if ((prog < 0) || (prog >= 128)) {
    FLUID_LOG(FLUID_WARN, (char*) "Program number out of range");
    return NULL;
  }
  if ((synth->tuning == NULL) ||
      (synth->tuning[bank] == NULL) ||
      (synth->tuning[bank][prog] == NULL)) {
    FLUID_LOG(FLUID_WARN, (char*) "No tuning at bank %d, prog %d", bank, prog);
    return NULL;
  }
...
fluid_synth_create_tuning(fluid_synth_t* synth, int bank, int prog, const char* name)
{
  ...
  if (synth->tuning == NULL) {
    ...
    if (synth->tuning == NULL) {
      FLUID_LOG(FLUID_PANIC, (char*) "Out of memory");
      return NULL;
    }
			  ...
			  int len, int *key, double* pitch, int apply)
{
    ...
    if(!tuning)
        tuning = new_fluid_tuning ((char*) "Unnamed", bank, prog);
			   ...
			   char* name, int len, double* pitch)
{
  ...
  if (name) {
    snprintf(name, (size_t) len - 1, (char*) "%s", fluid_tuning_get_name(tuning));
...
fluid_synth_set_gen(fluid_synth_t* synth, int chan, int param, float value)
{
  ...
  if ((param < 0) || (param >= GEN_LAST)) {
    FLUID_LOG(FLUID_WARN, (char*) "Parameter number out of range");
    return FLUID_FAILED;
  }
		     ...
		     float value, int absolute, int normalized)
{
  ...
  fluid_channel_set_gen(synth->channel[chan], param, v, (char) absolute);
		      ...
		      int audio_chan, int midi_chan, int key, int vel)
{
  ...
  if ((key < 0) || (key >= 128)) {
    FLUID_LOG(FLUID_WARN, (char*) "Key out of range");
    return FLUID_FAILED;
  }

  if ((vel <= 0) || (vel >= 128)) {
    FLUID_LOG(FLUID_WARN, (char*) "Velocity out of range");
    return FLUID_FAILED;
  }
