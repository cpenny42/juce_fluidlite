...
fluid_hashtable_resize(fluid_hashtable_t *hash_table)
{
  ...
  new_size = (int) (3 * hash_table->size + 1);
...
/*   printf("%s: %d: resizing, new size = %d\n", __FILE__, __LINE__, new_size); */
  ...
  new_nodes = FLUID_ARRAY(fluid_hashnode_t*, (size_t) new_size);
  FLUID_MEMSET(new_nodes, 0, (size_t) new_size * sizeof(fluid_hashnode_t*));

  for (i = 0; i < hash_table->size; i++) {
    for (node = hash_table->nodes[i]; node; node = next) {
      next = node->next;
      hash_val = fluid_str_hash(node->key) % (unsigned int) new_size;
  ...
  hash_table->size = (unsigned int) new_size;
}
